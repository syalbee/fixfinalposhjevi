...
<script type="text/javascript">
    function resultHasil() {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo base_url('penjualan_grosir/readtotal'); ?>",
...
<script type="text/javascript">
    ...
    function addTochart() {
        console.log("Cek Btn");
        $.ajax({
            url: '<?= base_url('penjualan_grosir/add_to_cart'); ?>',
            type: "post",
            dataType: "json",
            data: $("#addTocartjual").serialize(),
            success: (a) => {
                $('#detail_cart').load("<?= site_url('penjualan_grosir/read'); ?>");
                resultHasil();
                console.log('s' + a);
            },
            error: (a) => {
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#kode_brg").on("input", function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('penjualan_grosir/get_barang'); ?>",
                data: kobar,
                success: function(msg) {
                    // console.log(msg);
                    $('#detail_barang').html(msg);
    ...
    $(document).ready(function() {
        ...
        $("#tblbarang").on('click', '.edit_cart', function() {
            ...
            $.ajax({
                url: "<?php echo base_url(); ?>penjualan_grosir/edit",
                method: "POST",
                data: {
                    diskon: brgdisc,
                    qty: brgqty,
    ...
    $(document).on('click', '.hapus_cart', function() {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>penjualan_grosir/remove",
            method: "POST",
            data: {
                row_id: row_id
            },
